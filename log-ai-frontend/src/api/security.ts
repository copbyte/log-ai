...
/**
 * 查询审计日志（分页，可按操作类型/用户筛选）
 * 后端 GET /api/audit，data 为分页对象 { records, total, current, size }
 */
export async function fetchAuditLogs(
  params: FetchAuditParams,
): Promise<{ records: AuditLogItem[]; total: number }> {
  const qs = new URLSearchParams()
  if (params.operation) qs.set('operation', params.operation)
  if (params.username) qs.set('username', params.username)
  if (params.page !== undefined) qs.set('page', String(params.page))
  if (params.size !== undefined) qs.set('size', String(params.size))
  const page = await request<PageResult<AuditLogItem>>(
    `${BASE_URL}/api/audit?${qs.toString()}`,
  )
  return { records: page.records, total: page.total }
}
